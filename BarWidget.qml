...
BarWidget {
  ...
  // Always occupy a status slot, even when there is nothing to update.
  visible: true
  ...
  BarIconButton {
    ...
    tooltipText: {
      ...
      if (root.checking) return "Checking AppMan…"
      if (root.count === 1) return "1 AppMan app installed"
      if (root.count > 1) return root.count + " AppMan apps installed"
      return "No AppMan apps installed"
    }
